"""
Pytest unit tests for core GraphData functionality.
"""
...
class TestGraphDataCreation:
    """Test GraphData object creation and initialization."""
    
    def test_create_with_array(self):
        """Test creating GraphData with numpy array."""
...
class TestGraphDataProperties:
    ...
    def test_dtype_property(self):
        """Test dtype property."""
        sig_int = GraphData(np.array([1, 2, 3]))
        sig_float = GraphData(np.array([1.0, 2.0, 3.0]))
        sig_complex = GraphData(np.array([1+2j, 3+4j]))
    ...
    def test_sample_rate_property(self):
        """Test sample_rate property extracts from ports."""
        sig_with_sr = GraphData(np.array([1.0]), metadata={'sample_rate': 44100})
        sig_without_sr = GraphData(np.array([1.0]))
        
        assert sig_with_sr.sample_rate == 44100
        assert sig_without_sr.get('sample_rate') is None


class TestGraphDataCopy:
    """Test GraphData copy functionality."""
    
    def test_copy_creates_new_instance(self):
        """Test that copy creates a new instance."""
        data = np.array([1.0, 2.0, 3.0])
        metadata = {'sample_rate': 1000.0}
        sig1 = GraphData(data, metadata=metadata)
        sig2 = sig1.copy()
    ...
    def test_copy_is_deep(self):
        ...
        assert sig1.value == 42


class TestGraphDataScalarContainer:
    """Test using GraphData for scalar values (parameters)."""
    
    def test_scalar_as_array(self):
        """Test storing scalar as single-element array."""
        sig = GraphData(np.array([42.0]))
    ...
    def test_multiple_scalars(self):
        ...
        assert params.type == 'parameters'


class TestGraphDataComplexData:
    """Test GraphData with complex-valued data."""
    ...
    def test_amplitude_phase_reconstruction(self):
        ...
        amp = GraphData(np.abs(original), metadata={'type': 'amplitude'})
        phase = GraphData(np.angle(original), metadata={'type': 'phase'})
...
class TestGraphDataMultidimensional:
    """Test GraphData with multidimensional arrays."""
    
    def test_2d_array(self):
        """Test with 2D array (e.g., pulse matrix)."""
        data = np.random.randn(10, 100)  # 10 pulses, 100 samples each
        sig = GraphData(data, metadata={'type': 'pulse_matrix'})
